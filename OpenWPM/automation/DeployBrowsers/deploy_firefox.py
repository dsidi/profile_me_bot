...
def deploy_firefox(status_queue, browser_params, manager_params,
    ...
    if browser_params['extension_enabled']:
        ...
        # Adnauseum
        # Updated: 2019-10-1
        # ext_loc = os.path.join(root_dir, 'firefox_extensions', 'adnauseum',
        #                        'adnauseum-3.8.401.xpi')
        # ext_loc = os.path.realpath(ext_loc)
        # driver.install_addon(ext_loc, temporary=True)
        #
        # storage_dir = os.path.join(browser_profile_path, 'browser-extension-data')
        # adnauseum_dir = os.path.join(storage_dir, 'adnauseum@rednoise.org')
        # if not os.path.isdir(adnauseum_dir):
        #     os.mkdir(adnauseum_dir)
        # shutil.copy(os.path.join(root_dir, 'firefox_extensions',
        #                          'adnauseum', 'storage.js'), adnauseum_dir)
        # logger.debug("BROWSER %i: AdNauseum Firefox extension loaded (with settings)"
        #              % browser_params['crawl_id'])
